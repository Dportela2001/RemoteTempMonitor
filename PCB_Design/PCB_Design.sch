EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 7
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 1000 950  1200 850 
U 619FECCC
F0 "Battery Charge" 50
F1 "batteryCharge.sch" 50
F2 "Batt" I R 2200 1350 50 
$EndSheet
$Comp
L RF_Module:ESP32-WROOM-32 U1
U 1 1 61A64564
P 6050 3400
F 0 "U1" H 5950 3650 50  0000 C CNN
F 1 "ESP32-WROOM-32" H 5950 3550 50  0000 C CNN
F 2 "RF_Module:ESP32-WROOM-32" H 6050 1900 50  0001 C CNN
F 3 "https://www.espressif.com/sites/default/files/documentation/esp32-wroom-32_datasheet_en.pdf" H 5750 3450 50  0001 C CNN
	1    6050 3400
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0101
U 1 1 61A7545C
P 6050 4800
F 0 "#PWR0101" H 6050 4550 50  0001 C CNN
F 1 "GND" H 6055 4627 50  0000 C CNN
F 2 "" H 6050 4800 50  0001 C CNN
F 3 "" H 6050 4800 50  0001 C CNN
	1    6050 4800
	1    0    0    -1  
$EndComp
$Sheet
S 2700 950  1200 850 
U 619FB45F
F0 "Voltage Regulator" 50
F1 "VoltageRegulator.sch" 50
F2 "Batt" I L 2700 1350 50 
F3 "3.34V" O R 3900 1350 50 
$EndSheet
Text HLabel 4250 1350 2    50   Input ~ 0
3.3v
Wire Wire Line
	3900 1350 4250 1350
Text HLabel 6050 1800 1    50   Input ~ 0
3.3v
$Comp
L Device:R Ren
U 1 1 61A79C0C
P 5450 1900
F 0 "Ren" V 5350 1900 50  0000 C CNN
F 1 "10k" V 5450 1900 50  0000 C CNN
F 2 "" V 5380 1900 50  0001 C CNN
F 3 "~" H 5450 1900 50  0001 C CNN
	1    5450 1900
	0    1    1    0   
$EndComp
$Comp
L Switch:SW_Push SW1
U 1 1 61A7C25A
P 4900 2200
F 0 "SW1" H 4900 2350 50  0000 C CNN
F 1 "SW_Push" H 4900 2150 50  0000 C CNN
F 2 "" H 4900 2400 50  0001 C CNN
F 3 "~" H 4900 2400 50  0001 C CNN
	1    4900 2200
	1    0    0    -1  
$EndComp
Wire Wire Line
	6050 1800 6050 1900
Wire Wire Line
	5200 2200 5200 1900
Wire Wire Line
	5200 1900 5300 1900
Wire Wire Line
	5600 1900 6050 1900
Connection ~ 6050 1900
Wire Wire Line
	6050 1900 6050 2000
$Comp
L Switch:SW_Push SW2
U 1 1 61A853CB
P 7250 2200
F 0 "SW2" H 7250 2350 50  0000 C CNN
F 1 "SW_Push" H 7250 2150 50  0000 C CNN
F 2 "" H 7250 2400 50  0001 C CNN
F 3 "~" H 7250 2400 50  0001 C CNN
	1    7250 2200
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0102
U 1 1 61A9145D
P 7550 2200
F 0 "#PWR0102" H 7550 1950 50  0001 C CNN
F 1 "GND" V 7555 2072 50  0000 R CNN
F 2 "" H 7550 2200 50  0001 C CNN
F 3 "" H 7550 2200 50  0001 C CNN
	1    7550 2200
	0    -1   -1   0   
$EndComp
Wire Wire Line
	7450 2200 7550 2200
$Comp
L Device:LED LED_Prog_B
U 1 1 61A92BD7
P 6950 2900
F 0 "LED_Prog_B" H 6950 3000 50  0000 C CNN
F 1 "LED" H 6943 2736 50  0001 C CNN
F 2 "" H 6950 2900 50  0001 C CNN
F 3 "~" H 6950 2900 50  0001 C CNN
	1    6950 2900
	-1   0    0    1   
$EndComp
$Comp
L Device:R R_Prog_B
U 1 1 61AA10D3
P 7450 2900
F 0 "R_Prog_B" V 7550 2900 50  0000 C CNN
F 1 "47" V 7450 2900 50  0000 C CNN
F 2 "" V 7380 2900 50  0001 C CNN
F 3 "~" H 7450 2900 50  0001 C CNN
	1    7450 2900
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0103
U 1 1 61AA1C1E
P 7800 2900
F 0 "#PWR0103" H 7800 2650 50  0001 C CNN
F 1 "GND" H 7805 2727 50  0000 C CNN
F 2 "" H 7800 2900 50  0001 C CNN
F 3 "" H 7800 2900 50  0001 C CNN
	1    7800 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	7100 2900 7300 2900
Wire Wire Line
	7600 2900 7800 2900
Wire Wire Line
	6650 2200 7050 2200
Wire Wire Line
	6650 2900 6800 2900
$Comp
L power:GND #PWR0104
U 1 1 61AB247A
P 8000 2250
F 0 "#PWR0104" H 8000 2000 50  0001 C CNN
F 1 "GND" H 8005 2077 50  0000 C CNN
F 2 "" H 8000 2250 50  0001 C CNN
F 3 "" H 8000 2250 50  0001 C CNN
	1    8000 2250
	-1   0    0    1   
$EndComp
Wire Wire Line
	8000 2250 8100 2250
Wire Wire Line
	6850 2300 6850 2350
Wire Wire Line
	6850 2350 8100 2350
Wire Wire Line
	6650 2300 6850 2300
Wire Wire Line
	6650 2500 6850 2500
Wire Wire Line
	6850 2500 6850 2450
Wire Wire Line
	6850 2450 8100 2450
Text HLabel 8000 2550 0    50   Input ~ 0
3.3v
NoConn ~ 8100 2150
$Comp
L Connector:Conn_01x05_Male Conn_Prog
U 1 1 61AAC0B7
P 8300 2350
F 0 "Conn_Prog" H 8272 2282 50  0000 R CNN
F 1 "Conn_01x05_Male" H 8272 2373 50  0000 R CNN
F 2 "" H 8300 2350 50  0001 C CNN
F 3 "~" H 8300 2350 50  0001 C CNN
	1    8300 2350
	-1   0    0    1   
$EndComp
Wire Wire Line
	5200 2200 5450 2200
Wire Wire Line
	5100 2200 5200 2200
Connection ~ 5200 2200
$Comp
L power:GND #PWR0105
U 1 1 61AE9D34
P 4500 2200
F 0 "#PWR0105" H 4500 1950 50  0001 C CNN
F 1 "GND" H 4505 2027 50  0000 C CNN
F 2 "" H 4500 2200 50  0001 C CNN
F 3 "" H 4500 2200 50  0001 C CNN
	1    4500 2200
	1    0    0    -1  
$EndComp
Wire Wire Line
	4500 2200 4700 2200
$Sheet
S 7850 5400 500  200 
U 61B0DE61
F0 "Button2 " 50
F1 "Button.sch" 50
F2 "3.3v" I R 8350 5500 50 
F3 "Sw" O L 7850 5500 50 
$EndSheet
$Sheet
S 7850 4550 500  200 
U 61B0E45E
F0 "Button4" 50
F1 "Button.sch" 50
F2 "3.3v" I R 8350 4650 50 
F3 "Sw" O L 7850 4650 50 
$EndSheet
Wire Wire Line
	6650 5900 6650 4500
Wire Wire Line
	7850 5900 6650 5900
$Sheet
S 7850 5800 500  200 
U 61AF1821
F0 "Button1" 50
F1 "Button.sch" 50
F2 "3.3v" I R 8350 5900 50 
F3 "Sw" O L 7850 5900 50 
$EndSheet
Wire Wire Line
	6650 4300 6850 4300
Wire Wire Line
	6850 4300 6850 5100
Wire Wire Line
	6650 4200 6950 4200
Wire Wire Line
	6950 4200 6950 4650
Wire Wire Line
	6950 4650 7850 4650
Wire Wire Line
	6850 5100 7850 5100
$Sheet
S 7850 5000 500  200 
U 61B0D93C
F0 "Button3" 50
F1 "Button.sch" 50
F2 "3.3v" I R 8350 5100 50 
F3 "Sw" O L 7850 5100 50 
$EndSheet
Wire Wire Line
	7850 5500 6750 5500
Wire Wire Line
	6750 5500 6750 4400
Wire Wire Line
	6750 4400 6650 4400
Text HLabel 8950 5300 2    50   Input ~ 0
3.3v
Wire Wire Line
	8350 4650 8800 4650
Wire Wire Line
	8800 4650 8800 5100
Wire Wire Line
	8800 5300 8950 5300
Wire Wire Line
	8350 5100 8800 5100
Connection ~ 8800 5100
Wire Wire Line
	8800 5100 8800 5300
Wire Wire Line
	8350 5500 8800 5500
Wire Wire Line
	8800 5500 8800 5300
Connection ~ 8800 5300
Wire Wire Line
	8800 5500 8800 5900
Wire Wire Line
	8800 5900 8350 5900
Connection ~ 8800 5500
Text HLabel 8150 3600 0    50   Input ~ 0
3.3v
$Comp
L Connector:Conn_01x04_Female Conn_OLED
U 1 1 61A9EBD4
P 8450 3600
F 0 "Conn_OLED" H 8478 3576 50  0000 L CNN
F 1 "Conn_01x04_Female" H 8478 3485 50  0000 L CNN
F 2 "" H 8450 3600 50  0001 C CNN
F 3 "~" H 8450 3600 50  0001 C CNN
	1    8450 3600
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 61AA9827
P 8150 3400
F 0 "#PWR?" H 8150 3150 50  0001 C CNN
F 1 "GND" H 8155 3227 50  0000 C CNN
F 2 "" H 8150 3400 50  0001 C CNN
F 3 "" H 8150 3400 50  0001 C CNN
	1    8150 3400
	-1   0    0    1   
$EndComp
Wire Wire Line
	6650 3700 7600 3700
Wire Wire Line
	8250 3800 8050 3800
Wire Wire Line
	7750 3800 7750 3600
Wire Wire Line
	7750 3600 6650 3600
Wire Wire Line
	8250 3500 8150 3500
Wire Wire Line
	8150 3500 8150 3400
$Comp
L Connector:Conn_01x03_Female Conn_I2C
U 1 1 61B269DE
P 8450 4050
F 0 "Conn_I2C" H 8478 4076 50  0000 L CNN
F 1 "Conn_01x03_Female" H 8478 3985 50  0000 L CNN
F 2 "" H 8450 4050 50  0001 C CNN
F 3 "~" H 8450 4050 50  0001 C CNN
	1    8450 4050
	1    0    0    -1  
$EndComp
Wire Wire Line
	8250 3950 8050 3950
Wire Wire Line
	8050 3950 8050 3800
Connection ~ 8050 3800
Wire Wire Line
	8050 3800 7750 3800
Wire Wire Line
	8250 4050 7600 4050
Wire Wire Line
	7600 4050 7600 3700
Connection ~ 7600 3700
Wire Wire Line
	7600 3700 8250 3700
$Comp
L power:GND #PWR?
U 1 1 61B29741
P 8150 4150
F 0 "#PWR?" H 8150 3900 50  0001 C CNN
F 1 "GND" H 8155 3977 50  0000 C CNN
F 2 "" H 8150 4150 50  0001 C CNN
F 3 "" H 8150 4150 50  0001 C CNN
	1    8150 4150
	1    0    0    -1  
$EndComp
Wire Wire Line
	8250 4150 8150 4150
$Comp
L Device:LED LED_Piloto_G
U 1 1 61B3B199
P 7300 1400
F 0 "LED_Piloto_G" V 7339 1282 50  0000 R CNN
F 1 "LED" V 7248 1282 50  0000 R CNN
F 2 "" H 7300 1400 50  0001 C CNN
F 3 "~" H 7300 1400 50  0001 C CNN
	1    7300 1400
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR?
U 1 1 61B58AE1
P 7300 1550
F 0 "#PWR?" H 7300 1300 50  0001 C CNN
F 1 "GND" H 7305 1377 50  0000 C CNN
F 2 "" H 7300 1550 50  0001 C CNN
F 3 "" H 7300 1550 50  0001 C CNN
	1    7300 1550
	1    0    0    -1  
$EndComp
Text HLabel 7300 750  1    50   Input ~ 0
3.3v
$Comp
L Device:R R_Piloto
U 1 1 61B79D47
P 7300 1000
F 0 "R_Piloto" H 7370 1046 50  0000 L CNN
F 1 "180" H 7370 955 50  0000 L CNN
F 2 "" V 7230 1000 50  0001 C CNN
F 3 "~" H 7300 1000 50  0001 C CNN
	1    7300 1000
	1    0    0    -1  
$EndComp
Wire Wire Line
	7300 750  7300 850 
Wire Wire Line
	7300 1150 7300 1250
$Comp
L Device:LED_RGBC D?
U 1 1 61B93938
P 9300 2950
F 0 "D?" H 9300 2483 50  0000 C CNN
F 1 "LED_RGBC" H 9300 2574 50  0000 C CNN
F 2 "" H 9300 2900 50  0001 C CNN
F 3 "~" H 9300 2900 50  0001 C CNN
	1    9300 2950
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR?
U 1 1 61B96C97
P 9600 2950
F 0 "#PWR?" H 9600 2700 50  0001 C CNN
F 1 "GND" H 9605 2777 50  0000 C CNN
F 2 "" H 9600 2950 50  0001 C CNN
F 3 "" H 9600 2950 50  0001 C CNN
	1    9600 2950
	1    0    0    -1  
$EndComp
Wire Wire Line
	9500 2950 9600 2950
$Comp
L Device:R R_RGB_B
U 1 1 61BBF356
P 8850 2750
F 0 "R_RGB_B" V 8750 2750 50  0000 C CNN
F 1 "47" V 8850 2750 50  0000 C CNN
F 2 "" V 8780 2750 50  0001 C CNN
F 3 "~" H 8850 2750 50  0001 C CNN
	1    8850 2750
	0    1    1    0   
$EndComp
$Comp
L Device:R R_RGB_G
U 1 1 61BC376E
P 8850 2950
F 0 "R_RGB_G" V 8750 2950 50  0000 C CNN
F 1 "47" V 8850 2950 50  0000 C CNN
F 2 "" V 8780 2950 50  0001 C CNN
F 3 "~" H 8850 2950 50  0001 C CNN
	1    8850 2950
	0    1    1    0   
$EndComp
$Comp
L Device:R R_RGB_R
U 1 1 61BDC88F
P 8850 3150
F 0 "R_RGB_R" V 8750 3150 50  0000 C CNN
F 1 "180" V 8850 3150 50  0000 C CNN
F 2 "" V 8780 3150 50  0001 C CNN
F 3 "~" H 8850 3150 50  0001 C CNN
	1    8850 3150
	0    1    1    0   
$EndComp
Wire Wire Line
	9100 2750 9000 2750
Wire Wire Line
	9100 2950 9000 2950
Wire Wire Line
	9100 3150 9000 3150
Wire Wire Line
	8000 2550 8100 2550
Wire Wire Line
	8700 2750 8500 2750
Wire Wire Line
	8500 2750 8500 2600
Wire Wire Line
	8500 2600 6650 2600
Wire Wire Line
	8700 2950 8400 2950
Wire Wire Line
	8400 2950 8400 2700
Wire Wire Line
	8400 2700 6650 2700
Wire Wire Line
	8700 3150 8250 3150
Wire Wire Line
	8250 3150 8250 2800
Wire Wire Line
	8250 2800 6650 2800
Wire Wire Line
	8150 3600 8250 3600
$Comp
L Sensor_Temperature:DS18B20 Sens_Temp
U 1 1 61CA5F88
P 10400 3925
F 0 "Sens_Temp" H 10170 3971 50  0000 R CNN
F 1 "DS18B20" H 10170 3880 50  0000 R CNN
F 2 "Package_TO_SOT_THT:TO-92_Inline" H 9400 3675 50  0001 C CNN
F 3 "http://datasheets.maximintegrated.com/en/ds/DS18B20.pdf" H 10250 4175 50  0001 C CNN
	1    10400 3925
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 61CBF2E2
P 10400 4300
F 0 "#PWR?" H 10400 4050 50  0001 C CNN
F 1 "GND" H 10405 4127 50  0000 C CNN
F 2 "" H 10400 4300 50  0001 C CNN
F 3 "" H 10400 4300 50  0001 C CNN
	1    10400 4300
	1    0    0    -1  
$EndComp
Wire Wire Line
	10400 4300 10400 4225
Text HLabel 10400 3500 1    50   Input ~ 0
3.3v
Wire Wire Line
	10400 3500 10400 3625
$Comp
L Device:R 4.7k
U 1 1 61CCB79C
P 9925 3775
F 0 "4.7k" H 9995 3821 50  0000 L CNN
F 1 "R_OneWire" V 9825 3575 50  0000 L CNN
F 2 "" V 9855 3775 50  0001 C CNN
F 3 "~" H 9925 3775 50  0001 C CNN
	1    9925 3775
	1    0    0    -1  
$EndComp
Wire Wire Line
	9925 3925 10100 3925
Wire Wire Line
	9925 3625 10400 3625
Connection ~ 10400 3625
Text HLabel 9925 4025 3    50   BiDi ~ 0
One_Wire
Wire Wire Line
	9925 4025 9925 3925
Connection ~ 9925 3925
Text HLabel 6800 3100 2    50   BiDi ~ 0
One_Wire
Wire Wire Line
	6800 3100 6650 3100
$Comp
L Device:Buzzer Buzzer
U 1 1 61D29B96
P 7075 3850
F 0 "Buzzer" V 7125 3725 50  0000 L CNN
F 1 "Buzzer" H 7227 3788 50  0001 L CNN
F 2 "" V 7050 3950 50  0001 C CNN
F 3 "~" V 7050 3950 50  0001 C CNN
	1    7075 3850
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR?
U 1 1 61D2A2EE
P 7300 4475
F 0 "#PWR?" H 7300 4225 50  0001 C CNN
F 1 "GND" H 7300 4350 50  0000 C CNN
F 2 "" H 7300 4475 50  0001 C CNN
F 3 "" H 7300 4475 50  0001 C CNN
	1    7300 4475
	1    0    0    -1  
$EndComp
Wire Wire Line
	7300 4025 7300 3950
Wire Wire Line
	7300 3950 7175 3950
Wire Wire Line
	6650 4100 7000 4100
Wire Wire Line
	7000 4100 7000 4225
$Comp
L Device:Q_NMOS_GSD NMOS_Buzzer
U 1 1 61D5E461
P 7200 4225
F 0 "NMOS_Buzzer" H 7404 4271 50  0000 L CNN
F 1 "Q_NMOS_GSD" H 7404 4180 50  0000 L CNN
F 2 "" H 7400 4325 50  0001 C CNN
F 3 "~" H 7200 4225 50  0001 C CNN
	1    7200 4225
	1    0    0    -1  
$EndComp
Wire Wire Line
	7300 4475 7300 4425
Text HLabel 6975 4025 0    50   Input ~ 0
Batt
Wire Wire Line
	6975 4025 6975 3950
Wire Wire Line
	2200 1350 2450 1350
Text HLabel 2450 1475 3    50   Output ~ 0
Batt
Wire Wire Line
	2450 1475 2450 1350
Connection ~ 2450 1350
Wire Wire Line
	2450 1350 2700 1350
$EndSCHEMATC
